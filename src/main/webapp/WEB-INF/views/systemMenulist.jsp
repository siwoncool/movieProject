					...
					<tbody>
						<c:if test="${MODE eq 'admin'}">
							...
							<c:forEach var="price" items="${price}">
								<tr>
									<td><img width="130px" height="90px"
										src="resources/menu/${price.PImgName}"></td>
									<td>${price.PName}</td>
									<td>${price.PType}</td>
									<td>${price.PPrice}</td>
									<td>${price.PDc}</td>
									<td><input type="button" onclick="location.href='SMF019?pName=${price.PName}'" value="수정" class="btn btn-primary"/> <input type="button"class="btn btn-primary" onclick="location.href='SMF021?pName=${price.PName}'" value="삭제"/></td>
